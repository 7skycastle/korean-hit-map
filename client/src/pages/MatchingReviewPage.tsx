    ...
    <div className="space-y-5">
      <section className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">관리자가 등급과 공개 여부를 조정합니다. 승인된 케이스만 최종 리포트에 반영됩니다.</p>
        </div>
        <Link className="btn-primary" to={`/report/${report.id}`}>
          최종 리포트 보기
        </Link>
      </section>
      ...
      {report.cases.map((item) => (
        <section key={item.id} className="space-y-3">
          <div className="flex flex-wrap items-center gap-3 rounded-md border border-slate-200 bg-white p-3 shadow-report">
            ...
            <input
              className="field min-w-80 flex-1"
              value={item.aiSummary}
              onChange={(e) =>
                setReport({
                  ...report,
                  cases: report.cases.map((row) => (row.id === item.id ? { ...row, aiSummary: e.target.value } : row))
                })
              }
              onBlur={(e) => patch(item, { aiSummary: e.target.value })}
            />
          </div>
