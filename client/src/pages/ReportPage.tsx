...
export default function ReportPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-md border border-slate-200 bg-white p-6 shadow-report">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-4xl text-slate-600">이 리포트는 단순 키워드 일치가 아니라, 실제 시험장에서 체감 가능한 작품·제재·핵심 개념·문항 구조·선지 판단 기준의 연결성을 분석합니다.</p>
          </div>
          <Link className="btn-secondary" to={`/review/${approvedReport.id}`}>검수 화면</Link>
        </div>
      ...
      <div className="space-y-6">
        {approvedReport.cases.map((item) => <EvidenceCompareCard key={item.id} item={item} />)}
      </div>

      <p className="rounded-md bg-slate-200 px-4 py-3 text-sm font-semibold text-slate-600">본 분석은 동일 문항 출제를 의미하지 않습니다. 학생이 시험장에서 체감할 수 있는 연계 요소와 학습 효과를 분석한 자료입니다.</p>
    </div>
  );
}
